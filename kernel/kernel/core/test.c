...
void test(void) 
{
    void *a = kmalloc(0x1000);
    kprintf("%lX\n",vmm_get_physical((uintptr_t)a));

    kprintf("[BOOT] A varrer dispositivos de armazenamento à procura de partições...\n");

    // 5. Recupera o ponteiro do disco rígido bruto que o AHCI registou
    block_device_t* hd_bruto = block_get_device_by_name("ahci0");

    if (hd_bruto != NULL) {
        // 6. Faz o SCAN automático do disco!
        partition_scan_device(hd_bruto);
    } else {
        kprintf("[BOOT] Erro Crítico: Unidade fisica 'ahci0' não encontrada no catálogo.\n");
    }
    ...
    int status = vfs_mount("ahci0.1", "/", "fat32");
